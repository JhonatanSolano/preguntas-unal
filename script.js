...
function elementoPuedeScrollVertical(el, deltaY) {
...
function ancestroScrollableVertical(start, deltaY) {
  let node = start?.nodeType === Node.ELEMENT_NODE ? start : start?.parentElement;
  while (node && node !== document.body && node !== document.documentElement) {
    if (elementoPuedeScrollVertical(node, deltaY)) return node;
...
function desplazarSeccionActiva(deltaY) {
  ...
  if (!elementoPuedeScrollVertical(root, deltaY) && root !== document.scrollingElement && root !== document.documentElement) return false;
  root.scrollBy({ top: deltaY, left: 0, behavior: "auto" });
  return true;
}
function instalarPuenteScrollTrackpad() {
  if (window.__matematicasTrackpadScrollBridge) return;
  window.__matematicasTrackpadScrollBridge = true;
  let touchY = 0;
  document.addEventListener("wheel", event => {
    if (!event.deltaY || event.ctrlKey || event.defaultPrevented) return;
    const target = event.target;
    if (esZonaScrollNativa(target)) return;
    const root = seccionScrollableActiva();
  ...
  document.addEventListener("touchmove", event => {
    ...
    if (esZonaScrollNativa(target)) return;
    const root = seccionScrollableActiva();
    const nativeScroller = ancestroScrollableVertical(target, deltaY);
    if (nativeScroller && nativeScroller !== root) {
      touchY = currentY;
      return;
    }
